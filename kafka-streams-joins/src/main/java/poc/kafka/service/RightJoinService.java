...
public class RightJoinService {
	...
	private void streamStreamJoin() {
		...
		KStream<Long, CustomerOrder> joinedStream = customerStream.leftJoin(orderStream,
				(customer, order) -> new CustomerOrder(customer, order), JoinWindows.of(Duration.ofSeconds(120)),
				Joined.with(longSerde, customerSerde, orderSerde));
	...
	private Properties configs() {
		...
		configs.put("application.id", "kafka-streams-joins-right");
